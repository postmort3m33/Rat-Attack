...
local plasmaGunHeatBarFrame = thisPlayer:WaitForChild("PlayerGui").MainGUI.HUDFrame.MainFrame.WeaponGUIFrame.PlasmaGunFrame.HeatBarFrame.HeatBar

-- Flamethrower Gas Vars..
local FLAMETHROWER_MAX_AMMO = 500
local flamethrowerGas = FLAMETHROWER_MAX_AMMO
local flamethrowerGasBarFrame = thisPlayer:WaitForChild("PlayerGui").MainGUI.HUDFrame.MainFrame.WeaponGUIFrame.FlamethrowerFrame.GasBarFrame.GasBar

-- Crossbow Vars:
local crossBowLoaded = false
local CROSSBOW_FIRE_RELOAD_DELAY = 0.33
local crossBowAmmoFrame = thisPlayer:WaitForChild("PlayerGui").MainGUI.HUDFrame.MainFrame.WeaponGUIFrame.CrossbowFrame

-- Nailgun Vars
local NAILGUN_MAX_AMMO = 30
local nailGunAmmo = NAILGUN_MAX_AMMO
local nailGunAmmoFrame = thisPlayer:WaitForChild("PlayerGui").MainGUI.HUDFrame.MainFrame.WeaponGUIFrame.NailgunFrame

-- Shotgun Vars
local SHOTGUN_MAX_AMMO = 8
local SHOTGUN_FIRE_RELOAD_DELAY = 0.15
local shotgunAmmo = SHOTGUN_MAX_AMMO
local shotgunAmmoFrame = thisPlayer:WaitForChild("PlayerGui").MainGUI.HUDFrame.MainFrame.WeaponGUIFrame.ShotgunFrame

-- BBGun Vars
local BBGUN_MAX_AMMO = 50
local bbGunAmmo = BBGUN_MAX_AMMO
local bbGunAmmoFrame = thisPlayer:WaitForChild("PlayerGui").MainGUI.HUDFrame.MainFrame.WeaponGUIFrame.BBGunFrame

-- Vial Gun Vars..
local VIALGUN_MAX_AMMO = 150
local vialGunAmmo = VIALGUN_MAX_AMMO
local vialGunAmmoBar = thisPlayer:WaitForChild("PlayerGui").MainGUI.HUDFrame.MainFrame.WeaponGUIFrame.VialGunFrame.AmmoBarFrame.AmmoBar

-- Sounds --
local sipDrinkSound = script.Parent:WaitForChild("SipDrink")
local sodaCanOpenSound = script.Parent:WaitForChild("SodaCanOpen")
...
local STAMINA_REGEN_COOLDOWN = 1.5
local staminaCanRegen = true
local lastStaminaRegen = time()
local currentStaminaFrame = thisPlayer:WaitForChild("PlayerGui").MainGUI.HUDFrame.MainFrame.StaminaFrame.CurrentStaminaFrame
...
-- Reload Function --
local function Reload(toolName)
	...
	elseif toolName == "Shotgun" then
		...
		if shotgunAmmo < SHOTGUN_MAX_AMMO then
			...
			-- Load and Play Reload Animation if active
			if viewModel and activeTool then
				if activeTool.Name == toolName then
			...
			for i = 0, SHOTGUN_MAX_AMMO - 1, 1 do
				...
				-- If we lost the tool, break loop..
				if activeTool and viewModel then
					if activeTool.Name ~= toolName then
						break
					end
				...
				if shotgunAmmo >= SHOTGUN_MAX_AMMO then
					...
					-- Set to max Ammo before breaking..
	...
	elseif toolName == "Frag" or toolName == "Cheese" then
		...
		-- Hide Local Model
		if activeTool and viewModel and item then
	...
	elseif toolName == "Energy" then
		
		-- If we were sprinting, stop..
		DeactivateSprint()
		
		-- Open Can
		sodaCanOpenSound:Play()
...
local function ActivateTool()
	...
	-- If Energy
	if activeTool.Name == "Energy" then
...
local function ReloadButtonPressed(actionName, inputState, inputObj)
	...
	if inputState == Enum.UserInputState.Begin then
		...
		if not reloadFunctionRunning then
			...
			-- Reload
			if activeTool.Name ~= "PlasmaGun" and activeTool.Name ~= "Frag" and activeTool.Name ~= "Cheese" and activeTool.Name ~= "Coffee" and activeTool.Name ~= "Flashlight" and activeTool.Name ~= "Energy" then
...
thisCharacter.ChildAdded:Connect(function(child)
	...
	elseif child.Name == "Gun" then
		...
		weapon = game.ReplicatedStorage.GunSystem.Guns[weaponString].Model:Clone()
	...
	elseif child.Name == "Item" then
		...
		-- Set Weapon Reference..
		item = game.ReplicatedStorage.GunSystem.Items[child.Value.Settings.Model.Value].Model:Clone()
...
-- Turn off Flamethrower when Weapons Bench is opened..
game.ReplicatedStorage.PlayerOpenedWeaponsBench.OnClientEvent:Connect(function()
	
	-- If we are aiming a weapon, unaim it..
	if aiming then
	...
	-- If flamethrower is on, turn it off.
	if flamethrowerOn then
...
-- PLayer opened Bag
game.ReplicatedStorage.PlayerOpenedBagCTC.Event:Connect(function()
	...
	if activeTool then
		...
		elseif activeTool.Name == "CrossbowExplosive" then
			...
			if crossBowLoaded then
				...
				-- Set it
				crossBowAmmoFrame.CurrentAmmo.Text = 1

			else
				
				-- Set it
				crossBowAmmoFrame.CurrentAmmo.Text = 0
				
			end
			
			-- Set max
			crossBowAmmoFrame.MaxAmmo.Text = 1
