...
local maxRats = 0
local maxRatsMultiplier = 18 -- Default: 17. Max Rats Basewd on Number of players..
...
--local ratHealthMultiplier = (maxHealth - startRatHealth) / math.sqrt(maxRounds - 1) -- Using SQRT Function..
...
-- local ratAlbinoHealthMultiplier = (maxRatAlbinoHealth - startRatAlbinoHealth) / math.sqrt(maxRounds - 1) -- Using SQRT Function..
...
local function SpawnRat(spawnAlbino, minSpeed, maxSpeed)
	...
	if spawnAlbino then
		...
		--local newHealth = math.floor((math.sqrt(CURRENT_ROUND - 1) * ratAlbinoHealthMultiplier) + startRatAlbinoHealth) -- Using SQRT Function
		...
		--local newHealth = math.floor((math.sqrt(CURRENT_ROUND - 1) * ratHealthMultiplier) + startRatHealth) -- Using SQRT
...
local function SetMaxRats()
	...
	if RATS_ON and GAME_STARTED then
		...
		maxRats = math.floor(math.sqrt(CURRENT_ROUND) * maxRatsMultiplier) -- 100 Rats at Round 34
...
game.ServerStorage.ServerEvents.StartGame.Event:Connect(function()
	...
	if numPlayers == 1 then
		maxRatsMultiplier = 17
	elseif numPlayers == 2 then
		maxRatsMultiplier = 18
	elseif numPlayers == 3 then
		maxRatsMultiplier = 19
	elseif numPlayers == 4 then
		maxRatsMultiplier = 20
	end
