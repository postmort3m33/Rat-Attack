...
local pathAgentParams = { -- PathFinding Parameters --
	AgentRadius = 0.5, -- Default: 1.1
	AgentHeight = 1, -- Default: 1.6
	AgentCanJump = true,
...
-- Cheese Vars
local cheeseThrownPart = nil
local cheeseThrown = false
local activeCheesePart = nil
local headToCheese = false
local distanceToCheese = 0
local cheeseAttractDistance = 50
...
-- Cheese was thrown in the workspace --
game.ReplicatedStorage.CheeseThrown.Event:Connect(function(thisCheese)

	-- Only define a new cheese if we dont have an active cheese 
	if not activeCheesePart then

		-- Define this cheese part --
		cheesePartThrown = thisCheese
		...
		-- New Cheese thrown when we are already Eating a cheese..
		
	end	
end)

-- Cheese destroyed --
game.ReplicatedStorage.CheeseDestroyed.Event:Connect(function(destroyedCheese)

	-- If it was same cheese that we were after..
	if activeCheesePart then
		
		-- If The Destroyed Cheese is the one we are on..
		if destroyedCheese == activeCheesePart then

			-- No Longer heading toward cheese --
			headToCheese = false
			activeCheesePart = nil
		end		
...
RunService.Heartbeat:Connect(function(delta)
	...
	if thisHumanoid.Health > 0 then
		...
		--------------------------
		-- Head to Cheese Logic --
		--------------------------			
		
		-- If some cheese was thrown..
		if cheeseThrown then
			
			-- Are we within distance?
			distanceToCheese = (cheesePartThrown.Position - thisRatPos).Magnitude

			-- If this rat is close enough..
			if distanceToCheese < cheeseAttractDistance and cheeseThrown then
				
				-- Stop Cheese thrown Loop --
				cheeseThrown = false
				
				-- Head to the cheese--
				headToCheese = true
				
				-- Set Active Cheese Part
				activeCheesePart = cheesePartThrown				
				
			end			
		end		
		
		-- If we need to be heading to a piece of cheese..
		if headToCheese and activeCheesePart then
			
			-- Are we within distance?
			local cheeseDirection = activeCheesePart.Position - thisRatPos
			distanceToCheese = cheeseDirection.Magnitude
			
			-- If we made it to the cheese.. Stop.
			if distanceToCheese < 4 then
				
				-- Make Cheese Play Sound
				if not activeCheesePart.Eating.IsPlaying then
					activeCheesePart.Eating:Play()					
				end			
				
				-- Stop and eat the cheese..
				thisHumanoid:MoveTo(thisRatPos)
				
			else -- keep heading to cheese --				
				
				-- Move
				thisHumanoid:Move(cheeseDirection)
		...
		else -- If not heading for cheese, find nearest player and attack them --
						...
						if distanceToPlayer <= attackDistance then
							...
							if distanceToPlayer <= jumpDistance then
								...
								if (time() - lastJump) >= jumpInterval then
									...
									-- Random Jump Power
									thisHumanoid.JumpPower = math.random(30,40)
